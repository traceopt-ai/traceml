...
to dataloader fetch, host-to-device transfer, compute, step overhead, or rank skew.
...
TraceML separates dataloader fetch, H2D, compute, and step overhead. Use that
split to avoid treating a transfer, compute, or overhead issue as a DataLoader
issue.
...
Use the compare output to check whether total step time, input time, step
overhead, or the diagnosis changed.
