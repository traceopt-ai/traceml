...
| Bounded in-memory deque tables for the live view, SQLite (WAL) for history | Cap per-rank memory yet keep a queryable history; renderers read from SQLite (rejected: unbounded in-memory retention) | Oldest in-memory rows evict at `maxlen`; SQLite retains 30 minutes for analysis plus a fixed 5-minute late-arrival grace by aggregator receive time, configurable through `history_retention` |
...
| Bounded resource use | Memory and disk must not grow without limit during a long run | In-memory tables are deque-bounded; indexed SQLite pruning runs at most once per minute and at finalization, discarding raw rows beyond the configured logical horizon plus 5-minute grace |
