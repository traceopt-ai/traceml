...
def flush_traceml_buffers(attached: dict, model: nn.Module) -> None:
    ...
    if attached.get("activation_time"):
        flush_activation_time_buffers(model)    # TODO
    if attached.get("gradient_time"):
        flush_gradient_time_buffers(model)      # TODO
    if attached.get("execution"):
        flush_execution_buffers(model)          # TODO

def flush_activation_time_buffers(model: nn.Module):
    pass

def flush_gradient_time_buffers(model: nn.Module):
    pass
