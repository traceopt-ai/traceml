"""
Formatter implementations for TraceML run comparison.

The compare package intentionally separates:
- payload construction and verdict policy
- text formatting for CLI / artifact output

This keeps the compare command stable while allowing future renderers, such as
JSON-focused, HTML, or Rich formatters, to be added without changing verdict
semantics.
"""
...
class CompareTextFormatter(Formatter[Dict[str, Any], str]):
    """
    Format a structured compare payload as compact terminal-friendly text.

    The rendered report is intentionally split into:
    1. short conservative verdict
    2. material top changes only
    ...
    def format(self, payload: Dict[str, Any]) -> str:
        ...
        if isinstance(top_changes, list) and top_changes:
            ...
            _append_wrapped(
                lines,
                "- No material or clearly interpretable changes detected.",
            )
        ...
        wait_presence = _format_compare_presence(
            wait_share,
        ...
        if show_memory_section:
            ...
            if isinstance(mem_presented_rhs, dict) and mem_status.get(
                "changed"
            ):
