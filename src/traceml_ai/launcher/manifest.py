...
def _is_current_finalization_artifact(
    path: Path,
    *,
    aggregator_started_at: str,
) -> bool:
    """Return whether a valid artifact belongs to the current aggregator."""
    path = Path(path).resolve()
    if not path.is_file():
        return False
    ...
    try:
        return datetime.fromisoformat(completed_at) >= datetime.fromisoformat(
...
def read_current_finalization_reason(
    ...
    *,
    ...
    if _is_current_finalization_artifact(
