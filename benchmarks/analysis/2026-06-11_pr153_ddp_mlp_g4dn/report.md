...
- **Workload:** [`benchmarks/workloads/ddp_mlp_e2e.py`](../../workloads/ddp_mlp_e2e.py)
...
Single-GPU is rock-steady. The high DDP CV% is **not** measurement noise —
it is the network burst-credit drop after repeat 1 (see §5); the paired,
...
2. **Rank-divergent stop can hang multi-node runs.** In DDP both ranks must
   ...
   `dist.all_reduce(stop, op=ReduceOp.MAX)` so all stop on the same step —
   ~3 lines, mode-symmetric so it doesn't bias the comparison); or
