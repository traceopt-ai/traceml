...
def _rank(
    *,
    ...
    return RankStepSummary(
        ...
        avg_model_step_ms=effective_step,
...
def test_step_time_compute_bound_card_uses_short_reason() -> None:
    ...
    assert (
        "- Stats: total 97.0ms | model 95.0ms | compute 90.0ms"
        in payload["card"]
    )
    assert (
        "- Why: Compute dominated (90.0ms/95.0ms); backward was largest."
        in payload["card"]
    )
    _assert_compact_card(payload["card"])
...
def test_step_time_wait_heavy_card_uses_short_reason() -> None:
    ...
    assert (
        "- Why: WAIT* was high inside the traced step (30.0ms/100.0ms)."
        in payload["card"]
    )
    _assert_compact_card(payload["card"])
