...
| `lightning_dataloading_bottleneck.ipynb` | The PyTorch Lightning path: diagnose and fix the same real-image input bottleneck with `TraceMLCallback`, ResNet-18, and 320px Imagenette, compare the two runs with `traceml compare`, then read the per-step Input Wait to see the cold first batch of every epoch that the run average hides | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/traceopt-ai/traceml/blob/main/notebooks/lightning_dataloading_bottleneck.ipynb) |
